...
/**
 * 90% of this function was adapted from gxEPD WiFi example all credits go to ZinggJM
 */
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED && connectTries<16) {
    Serial.print(" .");
    delay(500);
    connectTries++;
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
   ...
   //New function that reads the URL set in Config.h
